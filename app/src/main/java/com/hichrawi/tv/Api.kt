...
object Api {
    ...
    data class ContentItem(val id: Long, val title: String, val posterUrl: String?, val streamUrl: String?, val category: String)
    ...
    fun content(context: Context, deviceId: Long, type: String): List<ContentItem> {
        return try {
            val obj = request(context, "/api/v1/content.php?device_id=$deviceId&type=$type")
            val arr = obj.optJSONArray("items") ?: return emptyList()
            (0 until arr.length()).mapNotNull { i ->
                val x = arr.optJSONObject(i) ?: return@mapNotNull null
                ContentItem(
                    x.optLong("id", i.toLong()),
                    x.optString("title", ""),
                    x.optString("poster_url").takeIf { it.isNotBlank() },
                    x.optString("stream_url").takeIf { it.isNotBlank() },
                    type
