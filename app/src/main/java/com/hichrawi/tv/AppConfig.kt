...
object AppConfig {
    ...
    /** Optional bootstrap: lets the same APK move to a new VPS later. */
    fun refreshRemote(context: Context) {
        try {
            ...
            client.newCall(req).execute().use { response ->
                ...
                val base = JSONObject(body).optString("api_base").trim().trimEnd('/')
        ...
        } catch (_: Exception) { /* fallback remains available */ }
    }

    fun deviceKey(context: Context): String {
        val p = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        var value = p.getString("device_key", null)
        if (value == null) {
            value = UUID.randomUUID().toString()
            p.edit().putString("device_key", value).apply()
        }
        return value
    }

    fun serverDeviceId(context: Context): Long =
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).getLong("server_device_id", 0L)

    fun saveServerDeviceId(context: Context, id: Long) =
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().putLong("server_device_id", id).apply()
}
