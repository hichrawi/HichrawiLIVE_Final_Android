...
window.saveChannel=async()=>{
 ...
 if(channelKey.length===0){
  if(window.crypto?.getRandomValues){const bytes=new Uint8Array(13);crypto.getRandomValues(bytes);for(const b of bytes)channelKey+=alphabet[b%alphabet.length];}
  else for(let i=0;i<13;i++)channelKey+=alphabet[Math.floor(Math.random()*alphabet.length)];
 }
...
function generateSubscriptionCode(){
  ...
  if(window.crypto?.getRandomValues){
    const bytes=new Uint8Array(13); crypto.getRandomValues(bytes);
    for(const b of bytes) code += alphabet[b % alphabet.length];
  }else{
    for(let i=0;i<13;i++) code += alphabet[Math.floor(Math.random()*alphabet.length)];
  }
  return code;
}
...
window.createSubscription=async()=>{
  ...
  if(code.length!==13){ alert("❌ كود الاشتراك يجب أن يكون 13 خانة بالضبط."); return; }
  try{
